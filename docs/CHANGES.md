...
7. Dead Keys & Keyboard layouts
    ...
    - The virtual key conversions weren't always correct due to the thread responsible for converting them not knowing what the original thread's keyboard layout is. To work around this, we do the conversion within the key handling function like winex11 does.
        - For a more upstreamable solution we should make it such that we can grab the other thread's keyboard layout. However, we would need to deal with getting the syncronization correct which could be tricky. The 5 line solution was a lot more appealing and was quick to implement.
